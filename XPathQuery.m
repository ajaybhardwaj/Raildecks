...
NSDictionary *DictionaryForNode(xmlNodePtr currentNode, NSMutableDictionary *parentResult)
{
	...
	if (currentNode->name)
    {
		...
		currentNodeContent= [NSString stringWithCString:(const char *)currentNode->name encoding:NSUTF8StringEncoding];
	...
	if (currentNode->content && currentNode->content != (xmlChar *)-1)
    {
		...
		if ([[resultForNode objectForKey:@"nodeName"] isEqual:@"text"] && parentResult&&currentNodeContent)
        {
			[parentResult
			 setObject:
			 [currentNodeContent
