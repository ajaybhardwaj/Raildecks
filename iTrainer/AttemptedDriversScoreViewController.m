//
//  AttemptedDriversScoreViewController.m
//  iTrainer
//
...
@interface AttemptedDriversScoreViewController ()

@end

@implementation AttemptedDriversScoreViewController
...
- (void) submitScoreToServerInBackground {
    ...
    for (int i=0; i<delegate.SELECTEDDRIVERTESTRESULTARRAY.count/2; i++) {
        
//        NSString *urlString = [NSString stringWithFormat:@"http://54.200.182.176/api/submitscore.php?driverid=%d&moduleid=%@&iscompleted=%@&score=%@",delegate.DRIVERID,[delegate.SELECTEDDRIVERTESTRESULTARRAY objectAtIndex:(i*2)],@"1",[delegate.SELECTEDDRIVERTESTRESULTARRAY objectAtIndex:(i*2)+1]];
        
        NSString *urlString;
        
        if (i==delegate.SELECTEDDRIVERTESTRESULTARRAY.count/2 - 1) {
            urlString = [NSString stringWithFormat:@"http://54.200.182.176/api/submitscore.php?driverid=%d&moduleid=%@&iscompleted=%@&score=%@&trainerid=%@",delegate.DRIVERID,[delegate.SELECTEDDRIVERTESTRESULTARRAY objectAtIndex:(i*2)],@"1",[delegate.SELECTEDDRIVERTESTRESULTARRAY objectAtIndex:(i*2)+1],delegate.TRAINERUSERNAME];
        }
        else {
            urlString = [NSString stringWithFormat:@"http://54.200.182.176/api/submitscore.php?driverid=%d&moduleid=%@&iscompleted=%@&score=%@",delegate.DRIVERID,[delegate.SELECTEDDRIVERTESTRESULTARRAY objectAtIndex:(i*2)],@"1",[delegate.SELECTEDDRIVERTESTRESULTARRAY objectAtIndex:(i*2)+1]];
        }
...
/*-------------------------- For Moving To Module View Controller ------------------------*/

- (void) moveToModuleViewController {
...
- (void) viewWillAppear:(BOOL)animated
{
    ...
    delegate.ISCOMINGBACKFROMATTEMPTEDDRIVERSSCOREVIEW = YES;
    
    unfinishedModulesCount = [delegate getModuleStatusRepeatingModulesOption];
    
    delegate.DRIVERNUMBEROFRECORDS = [delegate retrieveTotalNumberOfRecordsForDriver];
    
    [delegate retrieveSelectedDriverTestResultData];
    ...
    for (int i=0; i<delegate.SELECTEDDRIVERTESTRESULTARRAY.count/2; i++) {
        ...
        moduleLabel.text = [NSString stringWithFormat:@"Module %@",[delegate.SELECTEDDRIVERTESTRESULTARRAY objectAtIndex:(i*2)]];
        ...
        scoreLabel.text = [NSString stringWithFormat:@"%@%%",[delegate.SELECTEDDRIVERTESTRESULTARRAY objectAtIndex:(i*2)+1]];
    ...
    if (unfinishedModulesCount !=0 || delegate.DRIVERNUMBEROFRECORDS != delegate.MODULESCOUNT) {
        
        repeatModulesButton = [UIButton buttonWithType:UIButtonTypeCustom];
        repeatModulesButton.frame = CGRectMake(50, 250, 376, 75);
        ...
        [repeatModulesButton addTarget:self action:@selector(moveToModuleViewController) forControlEvents:UIControlEventTouchUpInside];
